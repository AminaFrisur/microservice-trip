...
#[wasm_bindgen(raw_module = "../../circuitBreaker.js")]
extern "C" {
    pub type CircuitBreaker;

    #[wasm_bindgen(method, catch)]
    pub async fn circuitBreakerRequest(this: &CircuitBreaker, path: String, bodyData: String, HeaderData: String, httpMethod: String) -> Result<JsValue,JsValue>;
}
...
impl Cache   {
    ...
    #[wasm_bindgen(method)]
    pub async fn check_and_get_booking_in_cache(&mut self, login_name: String, auth_token: String, buchungsnummer: i32, circuit_breaker: CircuitBreaker) -> CheckBookingCache {
        // let mut booking_found = false;
        ...
        if booking_index_i32 >= 0 {
            ...
            match circuit_breaker.circuitBreakerRequest(addr_with_params, login_name, auth_token, "GET".to_string()).await {
              Ok(res) => {
                  log("TEST".to_string());
